...
class FgICModel(pl.LightningModule):
    def __init__(self, cls_num, model_cfg):
        ...
        self.best_acc = 0.0
        self.print_freq = 50  # change this if the batch size is too large or small
    ...
    def on_train_batch_end(self, step_output, batch, batch_idx: int) -> None:
        ...
        loss, losses, cost = step_output['loss'], step_output['losses'], step_output['cost']
        if self.global_step % self.print_freq == 0:
            ...
            if self.local_rank == 0:
                ...
                print_info += 'Train Cost {:8.3f}'.format(cost)
                self.print(print_info)

    def on_validation_batch_end(
            self, step_out, batch, batch_idx: int, dataloader_idx: int=0,
    ) -> None:
        loss, losses, predictions, valid_cost = step_out
    ...
    def on_validation_epoch_end(self, ) -> None:
        ...
        valid_cost = self.valid_cost / self.trainer.num_val_batches[-1]
        if self.best_valid_cost > valid_cost:
            self.best_valid_cost = valid_cost
        self.log('BEST_COST', self.best_valid_cost, sync_dist=True)
        self.valid_cost = 0.0

        # record validation losses
        num_val_batches = self.trainer.num_val_batches[-1]

        valid_loss = functools.reduce(lambda x, y: x + y, [i for i, _, _, _ in outputs])
        multi_loss = functools.reduce(lambda x, y: x + y, [i for _, i, _, _ in outputs])
        ...
        print_info += ' |Val-C \033[0;32;40m{:.3f}\033[0m'.format(valid_cost)
        print_info += ' |Best-A \033[0;32;40m{:.3f}\033[0m'.format(self.best_acc * 100.)
        print_info += ' |Best-C \033[0;32;40m{:.3f}\033[0m'.format(self.best_valid_cost)
        self.print(print_info)
