    ...
    def __init__(self, input_cfg):
        ...
        self.train_trans = torchtfs.Compose(
            [
                ...
                torchtfs.RandomHorizontalFlip(),
                torchtfs.RandomRotation(15),
                torchtfs.ColorJitter(0.4, 0.4, 0.4, 0.1),
                torchtfs.ToTensor(),
                torchtfs.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5]),
                torchtfs.RandomErasing(p=0.1)  # 随机擦除
...
def build_transform(input_cfg):
    ...
    elif trans_name == 'Fathom':
        return FathomTransform(input_cfg)
    else:
